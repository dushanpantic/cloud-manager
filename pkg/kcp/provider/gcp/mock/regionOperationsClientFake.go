...
type RegionalOperationsClientFakeUtils interface {
	AddRegionOperation(projectID, region, name string) string
...
func (c *regionalOperationsClientFake) AddRegionOperation(projectID, region, name string) string {
	c.mutex.Lock()
	defer c.mutex.Unlock()

	key := projectID + "/" + region + "/" + name
...
func (c *regionalOperationsClientFake) GetRegionOperation(ctx context.Context, request client.GetRegionOperationRequest) (*computepb.Operation, error) {
	...
	key := request.ProjectId + "/" + request.Region + "/" + request.Name
